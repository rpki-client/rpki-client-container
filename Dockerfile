...
ENV BUILDREQ="git autoconf automake expat-dev libtool build-base fts-dev openssl-dev libtls-standalone-dev"

RUN set -x && \
  apk add --no-cache ${BUILDREQ} expat fts rsync tzdata tini && \
  cd /tmp && \
